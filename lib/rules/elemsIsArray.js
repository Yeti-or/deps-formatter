...
module.exports = (isArray, lint) => through.obj(function(file, _, next) {
    ...
    module.exports.rule({elemsIsArray: isArray, lint, errors: file.errors})(tree);
                ...
                if (parentEl.key.name === 'mustDeps' ||
                    ...
                    if (isArray) {
                        if (dep.type !== 'ArrayExpression') {
                            if (lint) {
                                errors.push('Always use array for ' + el.key.name, dep.parentElement.getSourceCode());
                            } else {
                        ...
                        if (dep.type === 'ArrayExpression' && dep.elements.length === 1) {
                            if (lint) {
                                errors.push('Don\'t use array for ' +
                                    el.key.name +
                                    ' where it is unnecessary:', dep.parentElement.getSourceCode());
                            } else {
                                dep = dep.elements[0];
