...
return function(tree) {
    ...
    // :deps() > ObjectExpression > ObjectProperty[key=elem|elems]:not([key=mod|mods]),
    // :deps() > ArrayExpression > ObjectExpression > ObjectProperty[key=elem|elems]:not([key=mod|mods])
    ...
    nodes.forEach(el => {
        ...
        if (el.key.name === 'elem' || el.key.name === 'elems') {
            ...
            if (
                $.getPropFromObject(el.parentElement, 'mod') ||
                $.getPropFromObject(el.parentElement, 'mods')
            ) { return; }
