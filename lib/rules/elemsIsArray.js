                ...
                if (parentEl.key.name === 'mustDeps' ||
                    ...
                    if (isArray) {
                        if (dep.type !== 'ArrayExpression') {
                            if (lint) {
                                var error = new Warnie('Always use array for ' + el.key.name);
                                var loc = dep.getLoc().start;
                                error.line = loc.line;
                        ...
                        if (dep.type === 'ArrayExpression' && dep.elements.length === 1) {
                            if (lint) {
                                var error = new Warnie('Don\'t use array for ' + el.key.name + ' where it is unnecessary');
                                var loc = dep.getLoc().start;
                                error.line = loc.line;
                                error.column = loc.column;
                                errors.push(error);
