...
function wrapExpressionInBrackets(tree, opts) {
    ...
    if (lint) {
        if (statement.expression.type !== 'ArrayExpression' &&
            ...
            error = new Warnie('Expected format: expression but get ' + statement.expression.type);
            loc = statement.expression.getLoc().start;
            error.line = loc.line;
    ...
    if (depObjs.length === 1) {
        ...
        if (lint) {
            if (statement.expression.type === 'ArrayExpression') {
                error = new Warnie('Expected format: expression' +
        ...
        if (lint) {
            if (statement.firstChild.value === '(') {
                ...
                loc = statement.firstChild.getLoc().start;
                error.line = loc.line;
                error.column = loc.column;
                errors.push(error);
            }
...
function wrapExpressionInRoundBrackets(tree, opts) {
    ...
    if (lint) {
        if (statement.expression.type !== 'ObjectExpression') {
            var error = new Warnie('Expected format: ObjectExpression but get ' + statement.expression.type);
            var loc = statement.firstChild.getLoc().start;
            error.line = loc.line;
            error.column = loc.column;
            errors.push(error);
        }
...
function wrapExpressionInArrayBrackets(tree, opts) {
    ...
    if (lint) {
        if (statement.expression.type !== 'ArrayExpression') {
            var error = new Warnie('Expected format: ArrayExpression but get ' + statement.expression.type);
            var loc = statement.firstChild.getLoc().start;
            error.line = loc.line;
            error.column = loc.column;
            errors.push(error);
...
function wrapExpressionInCommonJS(tree, opts) {
    ...
    if (lint) {
        if (statement.expression.type !== 'AssignmentExpression') {
            var error = new Warnie('Expected format: commonJS but get ' + statement.expression.type);
            var loc = statement.firstChild.getLoc().start;
            error.line = loc.line;
            error.column = loc.column;
            errors.push(error);
